---
type: company
ticker: TER
layer: 2
layer_name: Semiconductor Equipment
segment: ATE / semiconductor test
country: USA
---

# Teradyne  (TER)

> **Semiconductor Equipment** · ATE / semiconductor test · USA

**Role in the AI chain:** Test/ATE vendor with growing exposure to NVDA networking silicon test.

**Key products:** Automated test equipment.

## Driven by / benefits from
- [[Optical & Networking Buildout]] — (VIEW, UBS via TMTB 2026-05-29): NVDA networking test exposure; ATE annualized revenue could grow (?)

## Sources
- sources/inbox/TMTB：全面走强  EOD Wrap - TMT Breakout.pdf


## Added from research — 2026-06-09 (ingest)
- benefits from [[Co-Packaged Optics (CPO)]] — (VIEW, SemiAnalysis 2026-06-09): frontrunner in NVIDIA CPO test qualification; ficonTEC partnership

## Added from research — 2026-06-14 (ingest)
- benefits from [[HBM Demand]] — (VIEW, J.P. Morgan 2026-06-12 via Narrative Atlas): robust memory test (ATE) demand; multi-year memory-capex tailwind; 2nd hyperscaler ATE qualification
